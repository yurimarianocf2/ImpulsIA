-- =============================================================================
-- CORREÇÃO: TIPOS DE DADOS FOREIGN KEY
-- =============================================================================
-- Corrige incompatibilidade de tipos entre UUID e BIGINT
-- =============================================================================

-- 1. TABELA DE PERFIL DO CLIENTE (INALTERADA)
-- -----------------------------------------------------------------------------
...
-- 2. TABELA DE ANALYTICS DE CONSULTAS (CORRIGIDA)
-- -----------------------------------------------------------------------------
-- CORREÇÃO: produto_encontrado agora é BIGINT para coincidir com produtos.produto_id
...
CREATE TABLE consultas_precos (
    ...
    -- Dados da consulta - CORRIGIDO: BIGINT ao invés de UUID
    produto_pesquisado TEXT NOT NULL,
    produto_encontrado BIGINT REFERENCES produtos(produto_id), -- CORRIGIDO: BIGINT
    ...
    pedido_id UUID REFERENCES pedidos(id), -- Este continua UUID (correto)
...
-- 3. FUNÇÕES ATUALIZADAS COM TIPOS CORRETOS
-- -----------------------------------------------------------------------------
...
-- Função para registrar consulta de preços - CORRIGIDA
CREATE OR REPLACE FUNCTION registrar_consulta_preco(
    tel TEXT,
    produto TEXT,
    produto_id BIGINT DEFAULT NULL, -- CORRIGIDO: BIGINT
...
COMMENT ON TABLE cliente_perfil IS 'Perfil persistente do cliente - dados que sobrevivem além da sessão';
COMMENT ON TABLE consultas_precos IS 'Analytics de consultas de preços - tipos corrigidos';
...
COMMENT ON FUNCTION registrar_consulta_preco IS 'Registra consulta para analytics - tipos BIGINT/UUID corretos';
